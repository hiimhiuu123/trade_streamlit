...
# Nếu bạn sử dụng Streamlit Cloud, hãy lấy API key từ st.secrets; nếu không, bạn có thể dùng os.getenv()
# Ví dụ: MAP4D_API_KEY = st.secrets.get("MAP4D_API_KEY")
...
def render_map4d(df, api_key, map_id=""):
    ...
    Template cần chứa placeholder: ##MARKERS_PLACEHOLDER##, __API_KEY__, __MAP_ID__.
    """
...
def main():
    ...
    # Xử lý: Nếu cột 'name' trống, thay thế bằng bank_name + address
    df['name'] = df.apply(
    ...
    # Khi hiển thị bản đồ, chỉ dùng các dòng có giá trị hợp lệ của latitude, longitude
